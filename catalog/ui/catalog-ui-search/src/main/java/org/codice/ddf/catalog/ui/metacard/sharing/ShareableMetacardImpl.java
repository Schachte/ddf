...
public class ShareableMetacardImpl extends MetacardImpl {
  ...
  public static boolean isShareableMetacard(Metacard metacard) {
    // TODO: Can we imply shareable by just looking at the access attributes?
    return metacard != null
        && metacard.getTags().stream().anyMatch(ShareableMetacardAttributes.NAME::equals);

    //    List<AttributeDescriptor> shareableAttrs = new ArrayList<>();
    //
    //    if (metacard != null) {
    //      metacard
    //          .getMetacardType()
    ...
    //          .filter(
    //              attributeDescriptor ->
    //                  attributeDescriptor.getName().equals(ACCESS_GROUPS)
    //                      && attributeDescriptor.getName().equals(ACCESS_INDIVIDUALS))
    //          .forEach(shareableAttrs::add);
    //    }
    //
    //    return shareableAttrs.isEmpty();
  }
