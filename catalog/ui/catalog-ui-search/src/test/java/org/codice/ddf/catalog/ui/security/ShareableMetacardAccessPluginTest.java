...
public class ShareableMetacardAccessPluginTest {
  ...
  public void testSetOwnerRole() throws Exception {
    ...
    ShareableMetacardImpl before =
        ShareableMetacardImpl.from(ImmutableMap.of(Core.ID, id, Core.METACARD_OWNER, "user1"));

    ShareableMetacardImpl after =
        ShareableMetacardImpl.from(ImmutableMap.of(Core.ID, id, Core.METACARD_OWNER, "user2"));
