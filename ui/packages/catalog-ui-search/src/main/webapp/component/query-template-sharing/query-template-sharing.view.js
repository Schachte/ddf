...
define([
    ...
    var QueryTemplateSharing = Marionette.LayoutView.extend({
        ...
        getSharingByEmail: function () {
            return user.getQueryTemplateSecurity().get(this.options.templateId) != undefined ? 
                user.getQueryTemplateSecurity().get(this.options.templateId)['security.access-individuals'] 
                .map(function (email) {
                    return { value: email }
                }) : [];
        },
        getSharingByRole: function () {
            ...
            var roles = user.getQueryTemplateSecurity().get(this.options.templateId) != undefined ? 
                user.getQueryTemplateSecurity().get(this.options.templateId)['security.access-groups'] 
                .map(function (group) {
                    return group;
                }) : [];
        ...
        save: function () {
            ...
            /**
             * NEED TO HANDLE THE CASE IF THE KEY IN LIST IS NULL/NON-EXISTENT
             */
            var view = this;

            Loading.beginLoading(view);

            var templatePerms = user.getQueryTemplateSecurity();
        ...
        updateSharingPermissions: function(perms) {
            var _this = this;
            // TURN THIS INTO A PROMISE AND THEN EXECUTE THE USER PREFS SAVING AFTER THE SERVERSIDE GETS IT 
            // MAY NO LONGER WANT TO EVEN STORE INTO USER PREFS TBH
            $.ajax({
                url: perms.get('url').concat(_this.options.templateId),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                type: 'PUT',
                data: JSON.stringify(perms.get(_this.options.templateId)),
                success: function(data) {
                    announcement.announce({
                        title: 'Success!',
                        message: "Sharing Settings Saved for Query Template",
                        type: 'success'
                    });
                    _this.cleanup();
                },
                failure: function(data) {
                    announcement.announce({
                        title: 'Error!',
                        message: "There was a problem updating the sharing settings on this template",
                        type: 'error'
                    });
                }
            });
